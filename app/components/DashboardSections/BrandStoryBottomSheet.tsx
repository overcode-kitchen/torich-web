 ...
 * 형제 마진을 안 받아서다.
 */
export default function BrandStoryBottomSheet({
  isBrandStoryOpen,
  onCloseBrandStory,
}: BrandStoryBottomSheetProps) {
  useBodyScrollLock(isBrandStoryOpen)

  // 정적 export(prerender) 단계엔 document가 없다. 모든 호출부가 isBrandStoryOpen=false로
  // 시작하므로 이 가드로 하이드레이션 불일치 없이 서버 렌더만 건너뛴다.
  if (!isBrandStoryOpen || typeof document === 'undefined') return null
