...
- [x] **P2 전체** — "적립 정보" 라벨 / 알림 토글 / 헤더 통합 / 마감일 중복 제거 / 하드코딩 색 토큰화
  (※ 목적 추가 선택입력(P2-5)은 **의도적으로 제외** — 꼭 필요하지 않은 입력을 만들기 흐름에서 물으면 이탈한다.
  #70에서 다시 검토했고 같은 결론으로 닫았다. 근거는 `ux-consistency-audit.md` P2-5)
